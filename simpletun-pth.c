...
ser:
	...
	while(1) {
		...
		//	printf("TAP2NET %lu: Read %d bytes from the tap interface\n", tap2net, nread);
			...
			if( nwrite == -1)
				break;
//	fprintf(stderr,"write net %d\n",nwrite);

			//do_debug("TAP2NET %lu: Written %d bytes to the network\n", tap2net, nwrite);
		}
		for(int i=0;i<pth;i++)
			if(FD_ISSET(net[i], &rd_set)) {
				/* data from the network: read it, and write it to the tun/tap interface. 
				 * We need to read the length first, and then the packet */

				/* Read length */      
				nread = read_n(net[i], (char *)&plength, sizeof(plength));
					if(nread == 0) {
				...
				/* read packet */
				nread = read_n(net[i], buffer, ntohs(plength));
				//	fprintf(stderr,"read net %d\n",nread);
